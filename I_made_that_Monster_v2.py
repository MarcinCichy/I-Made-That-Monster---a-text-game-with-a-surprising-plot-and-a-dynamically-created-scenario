...
def main_game():
    ...
    room_number = 0                               
    while room_number != (len(list_of_rooms)):                                                              # With that loop I can make more and more rooms in 'description.txt' file
        is_used_items = len(used_items)
        room_obj = list_of_rooms[room_number]
        rooms[room_obj].show_room_description(is_used_items)
        show_menu()                                                 
        key_pressed = str(rooms[room_obj].choose_action(room_number, len(list_of_rooms), is_used_items))   # It would be too complicated for me to read this line if I used the function call instead of the variable 'room_obj'                          
            ...
            if key_pressed.lower() == "z":
                ...
                if len(equipment_taken) != 0:    
                   ...
                   time.sleep(2)
                    ...
                    time.sleep(2)      
    clear_screen() 
    if rooms[room_obj].name == list_of_rooms[-1]:
        show_description("LAST_DESCRIPTION", is_used_items)
        time.sleep(5)
    else:
        pass
    time.sleep(6)
