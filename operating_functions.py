...
def create_list_of_equipment(any_used_items):
    ...
    file_content = read_file(any_used_items)
    for num, line in enumerate(file_content):
        if "START" and ("ROOM" in line) and ("TRAP" not in line) and ("ITEM" not in line):
...
def end_game(name_of_room, rooms_count, any_used_items):
    if name_of_room == rooms_count[-1]:
        show_description("LAST_DESCRIPTION", any_used_items)
        time.sleep(5)
    clear_screen()
    ...
    if answer.lower() == "n":
        ...
        from I_made_that_Monster_v2 import main_game
        return main_game()
